...
MEMORY READ PATTERN: Brief calls are orientation only. Search calls are task-specific. For every non-trivial task you MUST run both steps: (1) project.brief + user.brief for general context, then (2) project.search + user.search with specific terms from this task: file names, function names, domain concepts, error messages. Do not rely on brief alone. Brief without search misses targeted knowledge.
